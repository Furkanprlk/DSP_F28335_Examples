...
//!
//! \note By default, this example project is configured for floating-point math. All
...
//!
//! - \b void \b SFO_MepEn(int i);  initialize MEP_Scalefactor[i] dynamically when HRPWM is in use.
//!
//! - \b void \b SFO_MepDis(int i);  initialize MEP_Scalefactor[i] when HRPWM is not used
//!
...
//!
//! -# 5MHz PWM (SYSCLK=150MHz) or 3.33MHz PWM (SYSCLK=100MHz), ePWM1A toggle low/high with MEP control on falling edge
//! -# 5MHz PWM (SYSCLK=150MHz) or 3.33MHz PWM (SYSCLK=100MHz) ePWM2A toggle low/high with MEP control on falling edge
//! -# 5MHz PWM (SYSCLK=150MHz) or 3.33MHz PWM (SYSCLK=100MHz) ePWM3A toggle high/low with MEP control on falling edge
//! -# 5MHz PWM (SYSCLK=150MHz) or 3.33MHz PWM (SYSCLK=100MHz) ePWM4A toggle high/low with MEP control on falling edge
//!
//!	\b Running \b the \b Application
//! -# Run this example at 150MHz SYSCLKOUT (or 100 MHz SYSCLKOUT for 100 MHz devices)
//! -# Load the Example_2833xHRPWM_SFO.gel and observe variables in the watch window
...
//! - EPwm4Regs.CMPA.all
//!
//! \note THE SFO.H FUNCTIONS INCLUDED WITH THIS EXAMPLE ONLY SUPPORTS EPWM1-EPWM4. FOR
