...
//!
//! \note By default, this example project is configured for floating-point math. All included libraries
...
//!
//! - \b int \b SFO_MepEn_V5(int i);   updates MEP_ScaleFactor[i] dynamically when HRPWM is in use.
//! - \b Returns
...
//!
//! - \b int \b SFO_MepDis_V5(int i);   updates MEP_ScaleFactor[i] when HRPWM is not used
//! - \b Returns
//! 	- 1 when complete for the specified channel
//! 	- 0 if not complete for the specified channel
//!
...
//! with HRPWM capabilities (default). Observe the duty cycle of the waveform changes in fine MEP steps
//! -# In the watch window:
//! Change the variable UpdateFine to 0, to observe the
//! ePWMxA output without HRPWM capabilities. Observe the duty cycle of the waveform changes in coarse steps of 10nsec.
//!
